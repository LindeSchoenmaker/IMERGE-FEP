...
parser.add_argument("-mid",
                    "--mid",
                    help="calc ddg halfway through production run",
                    type=bool,
                    default = False)
...
if __name__ == "__main__":
    ...
    for edge in edges:
        for leg in legs:
            for run in runs:
                ...
                if args.mid:
                    data_list_AB = [extract_u_nk(xvg, T=298.15).iloc[:round(len(extract_u_nk(xvg, T=298.15))/2)] for xvg in data_AB]
                else:
                ...
                if args.mid:
                    data_list_B = [extract_u_nk(xvg, T=298.15).iloc[:round(len(extract_u_nk(xvg, T=298.15))/2)] for xvg in data_B]
                else:
    ...
    with open(f"ddg_err_{condition}{'_mid' if args.mid else ''}.json", "w") as outfile: 
        json.dump(ddg_err_dict, outfile)

    with open(f"ddg_{condition}{'_mid' if args.mid else ''}.json", "w") as outfile: 
        json.dump(ddg_dict, outfile)

    with open(f"ddg_mbar_err_{condition}{'_mid' if args.mid else ''}.json", "w") as outfile: 
        json.dump(ddg_mbar_err_dict, outfile)

    with open(f"dg_{condition}{'_mid' if args.mid else ''}.json", "w") as outfile: 
        json.dump(dg_dict, outfile)

    with open(f"dg_err_{condition}{'_mid' if args.mid else ''}.json", "w") as outfile: 
